...
const cardTop = {
  ...
  navy: "from-blue-500 to-violet-500",
...
const avCls = {
  teal: "from-cyan-400 to-blue-600",
  blue: "from-blue-400 to-blue-700",
  green: "from-emerald-400 to-teal-700",
  purple: "from-purple-500 to-violet-700",
  indigo: "from-violet-500 to-purple-800",
...
function decorate(r) {
  ...
  return {
    ...r,
    _cs: hashPick(s, CARD_V),
    _as: hashPick(s + "av", AV_V),
    _ts: hashPick(s + "tg", TAG_V),
  };
}
...
function ToastItem({ id, type = "info", message, onRemove }) {
  ...
  return (
    ...
    >
      ...
      {/* icon */}
      <div
        className={`flex-shrink-0 w-8 h-8 rounded-xl ${cfg.iconBg} flex items-center justify-center`}
      >
      ...
      <div className="flex-1 pt-0.5 min-w-0">
        <p
          className={`poppins text-[0.78rem] font-semibold leading-snug ${cfg.title}`}
        >
          {message}
        </p>
      </div>
...
const ReviewCard = memo(function ReviewCard({ r }) {
  ...
  return (
    ...
    >
      <div
        className={`absolute top-0 left-5 right-5 h-[2px] rounded-b bg-gradient-to-r ${cardTop[r._cs]}`}
      />
      ...
      <div className="p-3.5 sm:p-5 pt-5 sm:pt-6 flex flex-col gap-2.5 relative z-10">
        ...
        <div className="flex items-center justify-between">
          <div className="flex gap-[3px]">
            {[1, 2, 3, 4, 5].map((i) => (
              <span
                key={i}
                className={`text-[13px] ${i <= stars ? "text-yellow-400" : "text-slate-700"}`}
              >
                ★
              </span>
            ))}
          ...
          <span
            className={`text-[0.6rem] font-bold tracking-widest uppercase px-2.5 py-[3px] rounded-full border ${tagCls[r._ts]}`}
          >
            {tag}
          </span>
        </div>
        ...
        <p className="text-[0.78rem] sm:text-[0.85rem] leading-relaxed text-slate-300/90 relative min-h-[56px] sm:min-h-[66px]">
          <span className="text-4xl leading-none text-cyan-400/15 font-black absolute -top-2 -left-1 poppins">
            "
          </span>
        ...
        <div className="flex items-center gap-2.5">
          <div
            className={`w-9 h-9 rounded-xl bg-gradient-to-br ${avCls[r._as]} flex items-center justify-center text-white text-xs font-bold flex-shrink-0`}
          >
            {initials}
          </div>
          <div>
            <p className="text-[0.82rem] font-semibold text-slate-100 poppins leading-tight">
              {name}
            </p>
...
const MarqueeRow = memo(function MarqueeRow({ reviews, direction }) {
  ...
  return (
    <div
      className={`flex gap-2.5 sm:gap-4 w-max py-2 ${direction === "fwd" ? "track-fwd" : "track-back"}`}
    >
      {doubled.map((r, i) => (
...
const INPUT_CLS =
  "w-full bg-white/[0.05] border rounded-xl px-4 py-2.5 text-sm text-white placeholder-blue-400/35 outline-none transition duration-200 focus:bg-cyan-950/30 focus:border-cyan-500/50 focus:ring-1 focus:ring-cyan-500/20";
...
function ParallaxBg({ children }) {
  ...
  return (
    <section
      ref={ref}
      className="relative w-full overflow-hidden bg-[#050B16] jakarta"
    >
      <motion.div
        style={{ y: bgY }}
        className="pointer-events-none absolute inset-0 opacity-[0.15] z-0 select-none"
      >
...
const handleSubmit = useCallback(async () => {
    ...
    setSubmitting(true);
    try {
      // ✅ FIXED: removed console.log(userId), added safe parsing
      const studentId = JSON.parse(localStorage.getItem("Student"))?.id;
      const facultyId = JSON.parse(localStorage.getItem("Faculty"))?.id;
      const userId = studentId || facultyId || null;

      const body = {
        name:    form.name.trim(),
        userId:  userId,                        // ✅ correctly defined now
  ...
  const avgRating = useMemo(
    () =>
      reviews.length
        ? (
            reviews.reduce((s, r) => s + Number(r.rating ?? r.stars ?? 5), 0) /
            reviews.length
          ).toFixed(1)
        : "—",
    [reviews],
  ...
  return (
    <>
      ...
      <Wrapper>
        {/* Dark fades */}
        <div
          className="pointer-events-none absolute top-0 inset-x-0 h-40 z-20"
          style={{
        ...
        <div className="relative z-10">
          ...
          <div className="text-center pt-20 pb-10 px-6">
            <Reveal dir="down">
              <div className="inline-flex items-center gap-2 bg-cyan-900/20 border border-cyan-700/25 rounded-full px-4 py-1.5 mb-5">
                ...
                <span className="text-cyan-400 text-[0.7rem] font-bold tracking-[0.12em] uppercase poppins">
                  Live Reviews
                </span>
              </div>
            </Reveal>
            ...
            <Reveal delay={0.18} dir="up">
              <p className="text-blue-200/70 text-sm max-w-md mx-auto leading-relaxed">
                Real voices from the Collegian&nbsp;X community — scholars,
            ...
            {[
              ...
              <Reveal key={s.label} delay={0.12 + i * 0.09} dir="scale">
                <div className="text-center">
                  <p className="poppins text-3xl font-extrabold text-cyan-400 tabular-nums">
                    {s.num}
                  </p>
                  <p className="text-blue-300/50 text-[0.68rem] uppercase tracking-widest mt-1">
                    {s.label}
                  </p>
                </div>
              </Reveal>
          ...
          {loading ? (
            ...
            <div className="marquee-wrap relative overflow-hidden pb-3">
              <div
                className="pointer-events-none absolute left-0 inset-y-0 w-28 z-10"
                style={{
                  background: "linear-gradient(90deg,#050B16,transparent)",
                }}
              />
              <div
                className="pointer-events-none absolute right-0 inset-y-0 w-28 z-10"
                style={{
                  background: "linear-gradient(270deg,#050B16,transparent)",
                }}
              />

              <Reveal dir="left">
                <div className="mb-4">
                  <MarqueeRow reviews={reviews} direction="fwd" />
                </div>
              </Reveal>
          ...
          <Reveal
            dir="up"
            delay={0.05}
            className="flex justify-center px-4 sm:px-6 py-16"
          >
            <div className="w-full max-w-2xl">
              <Reveal dir="up" delay={0.1}>
                <div className="text-center mb-8">
                  <h3 className="poppins text-2xl font-bold text-white mb-1.5">
                    Share Your Experience
                  </h3>
                  <p className="text-blue-200/60 text-sm">
                    Your voice helps the Collegian&nbsp;X community grow.
                  </p>
                </div>
              </Reveal>
              ...
              <Reveal dir="up" delay={0.18}>
                ...
                <div
                  className={`relative border border-white/[0.08] rounded-3xl p-7 md:p-9 overflow-hidden
                ...
                >
                  ...
                  {submitted && (
                    ...
                    >
                      ...
                      <p className="text-cyan-300 text-sm font-semibold poppins">
