...
const TeacherLeaveApproval = () => {
  ...
  const loadStatistics = useCallback(async () => {
    try {
      const response = await fetch(`${API_URL}/api/Faculty/Updateleave/statistics`, {
        credentials: 'include',
      });
  ...
  const loadApplications = useCallback(async (status) => {
    try {
      const response = await fetch(
        `${API_URL}/api/Faculty/Updateleave/applications?status=${status}`,
        {
          credentials: 'include',
        }
      );
  ...
  const handleApprove = useCallback(async (id, studentId) => {
    ...
    try {
      ...
      const response = await fetch(`${API_URL}/api/Faculty/Updateleave/approve/${id}`, {
        method: 'PUT',
  ...
  const confirmReject = useCallback(async (reason) => {
    ...
    try {
      ...
      const response = await fetch(`${API_URL}/api/Faculty/Updateleave/rejected/${currentRejectId.id}`, {
        method: 'PUT',
        headers: { 'Content-Type': 'application/json' },
        credentials: 'include',
        body: JSON.stringify({
